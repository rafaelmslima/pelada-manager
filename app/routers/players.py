...
@router.patch("/{player_id}/toggle-paid", response_model=schemas.PlayerRead)
...
def toggle_player_monthly(
    ...
    """Marca/desmarca a mensalidade do mes atual (mensalistas); atualiza o caixa."""
    pelada = get_current_pelada(current_user)
    player = crud.get_player(db, player_id, pelada.id)
    if player is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Jogador nao encontrado.")
    return crud.toggle_player_monthly_paid(db, pelada, player)


@router.patch("/{player_id}/toggle-daily", response_model=schemas.PlayerRead)
def toggle_player_daily(
    player_id: int,
    db: Session = Depends(get_db),
    current_user: models.User = Depends(require_user),
):
    """Marca/desmarca a diaria da pelada de hoje (diaristas); atualiza o caixa."""
    pelada = get_current_pelada(current_user)
    player = crud.get_player(db, player_id, pelada.id)
    if player is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Jogador nao encontrado.")
    return crud.toggle_player_daily_paid(db, pelada, player)
