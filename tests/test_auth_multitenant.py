...
class AuthMultitenantTest(unittest.TestCase):
    ...
    def test_deactivate_all_players_only_updates_current_pelada(self):
        engine = create_engine("sqlite:///:memory:", connect_args={"check_same_thread": False})
        SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
        Base.metadata.create_all(bind=engine)
        ...
        try:
            ...
            crud.create_player(
                db,
                schemas.PlayerCreate(name="Jogador B", position="ataque", rating=3.5, is_active=False),
                pelada_a.id,
            )
            crud.create_player(
                db,
                schemas.PlayerCreate(name="Jogador C", position="defesa", rating=4.2, is_active=True),
                pelada_b.id,
            )

            players_a = crud.deactivate_all_players(db, pelada_a.id)
            players_b = crud.get_players(db, pelada_b.id)
